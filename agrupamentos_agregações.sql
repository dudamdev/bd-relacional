...
SELECT cor FROM Animais GROUP BY cor ORDER BY cor;

SELECT cor, COUNT(ID)
FROM Animais
GROUP BY cor;

SELECT cor, COUNT(ID) AS Quantidade
FROM Animais
GROUP BY cor;

SELECT cor, AVG(peso) AS Média_Peso
FROM Animais
GROUP BY cor;

SELECT cor, AVG(peso) AS Peso_15plus
FROM Animais
GROUP BY cor
HAVING AVG(peso) > 15;

SELECT cor, SUM(peso) AS Soma_Peso
FROM Animais
GROUP BY cor;

SELECT nome, SUM(peso) AS Peso
FROM Animais 
GROUP BY nome;
...
SELECT nome_marca, COUNT(produtos.marca_id) AS qtd
from produtos JOIN marcas
on marcas.marca_id = produtos.marca_id
GROUP BY produtos.marca_id;